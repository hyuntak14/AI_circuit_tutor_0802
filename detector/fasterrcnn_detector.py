...
class FasterRCNNDetector:
    def __init__(self, model_path=None, num_classes=9):  # 8 classes including background
        ...
        if model_path:
            ...
            try:
                ...
                # If mismatch in keys (e.g., custom box_predictor), load non-strictly
                self.model.load_state_dict(checkpoint, strict=False)
        self.model.to(self.device).eval()

        self.class_names = [
            'background', 'Breadboard', 'Capacitor', 'Diode',
            'IC', 'LED', 'Line_area', 'Resistor'
        ]
    ...
    def detect(self, image):
        ...
        for label, box, score in zip(
                predictions['labels'], predictions['boxes'], predictions['scores']):
            if score < 0.5:
                continue
