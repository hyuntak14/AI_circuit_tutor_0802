...
###########################################
def interpolate_incomplete_rows_ref(row_groups, min_required=20, tolerance=10):
...
if __name__ == '__main__':
    ...
    best_threshold, best_groups, best_score = find_optimal_y_threshold(
        ...
        target_holes=50,  # 여기서 50은 완전 행의 기준으로 사용
    ...
    # 불완전한 행(구멍 20개 이상, 목표 미만)에 대해 보간 적용
    ...
    # (2) Scatter plot: 각 행별로 다른 색상으로 표시
    plt.figure(figsize=(8, 6))
    num_rows = len(interpolated_groups)
    for row_number, pts, median_y, is_interp in interpolated_groups:
        if pts:  # 행에 점이 있을 경우에만 처리
            xs, ys = zip(*pts)
            # 행 번호에 따라 0~1 범위의 값으로 정규화하고, colormap에서 색상 할당
            color = plt.cm.nipy_spectral(row_number / num_rows)
            plt.scatter(xs, ys, s=10, c=[color])
    plt.gca().invert_yaxis()
    plt.title("Scatter Plot of Holes by Row")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.grid(True)
    plt.show()
